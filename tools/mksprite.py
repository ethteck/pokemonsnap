...
def main():
    ...
    def emit_aligner(symbol: str):
        if args.aligner == "df":
            out_lines.append(
                f"static Gfx {symbol}_C_dummy_aligner[] = {{ gsSPEndDisplayList() }};"
            )
        else:
            out_lines.append(
                f"static u8 {symbol}_C_dummy_aligner[] = {{ 0, 0, 0, 0, 0, 0, 0, 0 }};"
            )
    ...
    for i, tile in enumerate(sprite.tiles):
        ...
        emit_aligner(tile_name)
        out_lines.append(f"{fmt_info['c_type']} {tile_name}[] = {{")
        out_lines.append(c_array)
        out_lines.append("};")
        out_lines.append("")

        if is_ci and sprite.palette is not None:
            tlut_name = f"{tile_name}tlut"
            emit_aligner(tlut_name)
            out_lines.append(f"unsigned short {tlut_name}[] = {{")
