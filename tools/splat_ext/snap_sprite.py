...
class N64SegSnap_sprite(Segment):
    ...
    def _resolve_rom(self, vram_addr: int) -> int:
    ...
    def split(self, rom_bytes: bytes):
        ...
        self.dl_name = self._resolve_name(header.rsp_dl, "dl") if header.rsp_dl else "NULL"
        self.bitmaps_name = self._resolve_name(header.bitmap, "bitmaps") if header.bitmap > 0x80000000 else f"{self.name}_bitmaps"

        # Render the sprite to PNG
        ...
        for bitmap in bitmaps:
            ...
            # swap the odd rows (word-swap for TMEM interleaving)
            if input_bpp >= 2:
