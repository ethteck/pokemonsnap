...
Overlay D_800ABBD0 = OVERLAY(unk_segment_AA18E0);
...
Overlay overlayCaveAssets = OVERLAY(cave_assets);
Overlay overlayCave = OVERLAY(cave_code);
Overlay overlayRiverAssets = OVERLAY(river_assets);
...
Overlay overlayValleyAssets = OVERLAY(valley_assets);
Overlay overlayValley = OVERLAY(valley_code);
Overlay overlayRainbowAssets = OVERLAY(rainbow_assets);
...
void func_8009A8F0(s32 levelID) {
    switch (levelID) {
        case SCENE_BEACH:
            ...
            dmaReadRom(magikarp_model_hd_ROM_START, magikarp_model_hd_VRAM, magikarp_model_hd_ROM_END - magikarp_model_hd_ROM_START);
            dmaReadRom(pikachu1_ROM_START, pikachu1_VRAM, pikachu1_ROM_END - pikachu1_ROM_START);
            dmaReadRom(pikachu_model_hd_ROM_START, pikachu_model_hd_VRAM, pikachu_model_hd_ROM_END - pikachu_model_hd_ROM_START);
            return;
        case SCENE_TUNNEL:
        ...
        case SCENE_CAVE:
            ...
            dmaReadRom(bulbasaur_model_hd_ROM_START, bulbasaur_model_hd_VRAM, bulbasaur_model_hd_ROM_END - bulbasaur_model_hd_ROM_START);
            dmaReadRom(magikarp_textures_ROM_START, magikarp_textures_VRAM, magikarp_textures_ROM_END - magikarp_textures_ROM_START);
            dmaReadRom(magikarp_model_hd_ROM_START, magikarp_model_hd_VRAM, magikarp_model_hd_ROM_END - magikarp_model_hd_ROM_START);
            dmaReadRom(pikachu1_ROM_START, pikachu1_VRAM, pikachu1_ROM_END - pikachu1_ROM_START);
            dmaReadRom(pikachu_model_hd_ROM_START, pikachu_model_hd_VRAM, pikachu_model_hd_ROM_END - pikachu_model_hd_ROM_START);
...
void start_scene_manager(s32 arg0) {
    ...
    dmaReadVPK0((u32*) intro_code_vpk0_ROM_START, VPK_VRAM);
    ...
    while (true) {
        ...
        switch (sceneId) {
            ...
            case SCENE_MAIN_MENU:
                setLevelId(SCENE_BEACH);
                func_800AAF20();
                dmaReadVPK0((u32*) main_menu_vpk0_ROM_START, VPK_VRAM);
