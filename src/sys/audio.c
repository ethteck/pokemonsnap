                ...
                case 1:
                    if (auBGMPlayers[i]->state != AL_STOPPED) {
                        ...
                        if (auBGMSongId[i] < 0) {
                            ...
                            auRomRead((u32)auSeqFile->seqArray[auBGMSongId[i]].offset, auBGMSeqData[i], auSeqFile->seqArray[auBGMSongId[i]].len);
                            auBGMPlayerStatus[i]++;
                        }
                    }
                    break;
...
void auSetCurrentSoundsGlobalVolume(u8 arg0) {
    ...
    for (i = 0; i < auCurrentSettings.numSounds; i++) {
        if (auSndpSoundId[i] != -1) {
            alSndpSetSound(auSoundPlayer, auSndpSoundId[i]);
            if (alSndpGetState(auSoundPlayer) == AL_PLAYING || auStartingSound[i] >= 0) {
                alSndpSetVol(auSoundPlayer, (auSoundVolume[i] * ((N_ALSoundState*)auSoundPlayer->sndState)[auSndpSoundId[i]].sound->sampleVolume * arg0) >> 14);
            }
        }
    }
    osSetIntMask(mask);
